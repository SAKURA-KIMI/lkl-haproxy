...
enable(){
	if [ "`cat /etc/issue | grep -iE "debian"`" ] || [ "`cat /etc/issue | grep -iE "ubuntu"`" ] || ([ -f "/etc/redhat-release" ] && [ "`cat /etc/redhat-release | grep -iE "centos"`" ])
	then
		wget --no-cache -O /etc/systemd/system/lkl-haproxy.service https://github.com/SAKURA-KIMI/lkxy/raw/master/requirement/lkl-haproxy.service
		systemctl daemon-reload
		systemctl enable lkl-haproxy
	elif [ "`cat /etc/issue | grep -iE "alpine"`" ]
	then
		command -v openrc || apk add openrc
		wget --no-cache -O /etc/init.d/lkl-haproxy https://github.com/SAKURA-KIMI/lkxy/raw/master/requirement/alpine/lkl-haproxy
...
config(){
	...
	[[ ! -f redirect.sh ]] && wget --no-cache -O redirect.sh https://github.com/SAKURA-KIMI/lkxy/raw/master/requirement/redirect.sh
...
check_all(){
	...
	# check lkl-mod
	[[ ! -f liblkl-hijack.so ]] && wget --no-cache https://github.com/SAKURA-KIMI/lkxy/raw/master/mod/liblkl-hijack.so
	...
	then
		wget -O ./haproxy https://github.com/SAKURA-KIMI/lkxy/raw/master/requirement/alpine/haproxy
